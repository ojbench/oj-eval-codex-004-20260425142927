    ...
    while(true){
        ...
        if(cmd=="su"){
            ...
            for(size_t i=1;i<tokens.size();++i){ string t=tokens[i]; if(t.rfind("-ISBN=",0)==0){ if(hasISBN){ invalid(); goto nextline; } hasISBN=true; string v=t.substr(6); if(!validAscii(v,20) || v==b->ISBN){ invalid(); goto nextline; } newISBN=v; }
                else if(t.rfind("-name=",0)==0){ if(hasName){ invalid(); goto nextline; } hasName=true; string v=t.substr(6); if(!(v.size()>=2 && v.front()=='"' && v.back()=='"')){ invalid(); goto nextline; } v=v.substr(1,v.size()-2); if(!validAsciiNoDQ(v,60) || v.empty()){ invalid(); goto nextline; } newName=v; }
                else if(t.rfind("-author=",0)==0){ if(hasAuthor){ invalid(); goto nextline; } hasAuthor=true; string v=t.substr(8); if(!(v.size()>=2 && v.front()=='"' && v.back()=='"')){ invalid(); goto nextline; } v=v.substr(1,v.size()-2); if(!validAsciiNoDQ(v,60) || v.empty()){ invalid(); goto nextline; } newAuthor=v; }
                else if(t.rfind("-keyword=",0)==0){ if(hasKeyword){ invalid(); goto nextline; } hasKeyword=true; string v=t.substr(9); if(!(v.size()>=2 && v.front()=='"' && v.back()=='"')){ invalid(); goto nextline; } v=v.substr(1,v.size()-2); if(!validAsciiNoDQ(v,60) || v.empty()){ invalid(); goto nextline; } // check duplicate segments
